...
import { useSwitchChildrenMutation } from "@/redux/endpoints/child";
...
const SwitchChildCard = ({ childData }: { childData: ChildProps }) => {
  const dispatch = useDispatch();
  const [switchChild, { data, isLoading, isError, isSuccess }] =
    useSwitchChildrenMutation();
  const { refetch: refetchWeight } = useGetWeightQuery(undefined);
  const { refetch: refetchHeight } = useGetHeightQuery(undefined);
  const { refetch: refetchLogs } = useGetLogsQuery(undefined);
  const { refetch: refetchMilestones } = useMilestoneCtegoriesQuery(undefined);
  const { refetch: refetchAchievements } =
    useGetAchievedMilestonesQuery(undefined);
  const { refetch: refetchDailyActivities } =
    useMilestoneDailyActivitiesQuery(undefined);

  const handleSwitch = async (e: any) => {
    try {
      const payload = await switchChild({ child_id: childData?.id });

      // @ts-ignore
      if (payload?.data?.success) {
        dispatch(invalidateMilestonesTags(["Milestones"]));
        // Manually refresh growth chart queries after switching children
        refetchWeight();
        refetchHeight();
        refetchLogs();
        refetchMilestones();
        refetchAchievements();
        refetchDailyActivities();
        window.location.reload();
        document.getElementById("switch-child-modal")?.click();
      }
    } catch (error) {
  ...
  return (
    <div>
      <div
        className={twMerge(
          "w-full md:p-[24px] p-[12px] rounded-[24px] border hover:border-primary-600 border-[#E2E6ED]",
...
export default SwitchChildCard;
